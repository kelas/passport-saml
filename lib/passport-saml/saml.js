...
SAML.prototype.generateUniqueID = function (req) {
  ...
  if (req) {
    uniqueID = req.headers.host + '-' + uniqueID;
  }
...
SAML.prototype.generateAuthorizeRequest = function (req, isPassive, callback) {
  var self = this;
  var id = "_" + self.generateUniqueID(req);
  ...
  .then(function(){
    var request = {
      'samlp:AuthnRequest': {
        ...
        '@ProtocolBinding': self.options.authnResponseBinding || 'urn:oasis:names:tc:SAML:2.0:bindings:HTTP-POST',
...
SAML.prototype.generateLogoutRequest = function (req) {
  var id = "_" + this.generateUniqueID(req);
...
SAML.prototype.checkTimestampsValidityError = function(nowMs, notBefore, notOnOrAfter) {
  ...
  if (notBefore) {
    ...
    if (nowMs + self.options.acceptedClockSkewMs < notBeforeMs) {
        var skew = notBeforeMs - nowMs;
        return new Error('SAML assertion not yet valid: local = ' + nowMs + ', notBefore = ' + notBeforeMs + ', acceptedSkew = ' + self.options.acceptedClockSkewMs + ', actualSkew = ' + skew);
    }
  }
  ...
  if (notOnOrAfter) {
    ...
    if (nowMs - self.options.acceptedClockSkewMs >= notOnOrAfterMs) {
      var skew = nowMs - notOnOrAfterMs;
      return new Error('SAML assertion expired: local = ' + nowMs + ', notOnOrAfter = ' + notOnOrAfterMs + ', acceptedSkew = ' + self.options.acceptedClockSkewMs + ', actualSkew = ' + skew);
    }
  }
